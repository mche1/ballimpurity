source("/gpfs/ysm/project/zhang_heping/mc3526/Ball_Impurity/simulation/km_median.R")
source("/gpfs/ysm/project/zhang_heping/mc3526/Ball_Impurity/simulation/splitDataset.R")
library(Rcpp)
sourceCpp("/vast/palmer/scratch/zhang_heping/mc3526/Ball_Imp/code/BallImpurity.cpp")
sourceCpp("/vast/palmer/scratch/zhang_heping/mc3526/Ball_Imp/code/BallDistanceVector.cpp")
lossFunc2 <- function(N,D,ind,d) {
  ## D为距离矩阵
  #  if (method == 'BallImpurity') {
  #n=dim(D)[1]
  #print(paste0("N=",N,", n=",n,"."))
  loss=BallImpurityFunc::BallImpurity(N,D,ind,d)
  return(loss)
  #    return(BallImpurity(N, D, 1:n, d))
  #  } else if (method == 'CART') {
  #    return(mean(D))
  #  } else {
  #    stop('Method error')
...
PruningStat1<-function(D,ind1){
  ## the first pruning statistic, in the spirit of ANOVA
...
# PruningStat2<-function(D,ind1,ind2,rep,d){
#   ## the 2nd pruning statistic, in the spirit of permutation test
#   ## D is the distance matrix of root node set (usually NOT the parent node unless it is the root node)
#   ## ind1 is the indices of either child node set in the parent node set, the rest forms another child node in a binary tree
#   ## ind2 is the indices of the other child node set
#   ind=c(ind1,ind2)
#   bivec=rep(0,rep)
...
#   m1=length(ind1)
#   m2=length(ind2)
#   BI_full=BallImpurity(N,D,ind,d)
#   BI_left=BallImpurity(N,D,ind1,d)
#   BI_right=BallImpurity(N,D,ind2,d)#BallImpurity(n,distance_matrix,split$rightInd,d)
#   bi=BI_full-m1/n*BI_left-m2/n*BI_right
#   for(i in 1:rep){
#     set.seed(i+1000)
#     indleft=sample(ind,size=length(ind1))
#     indright=setdiff(ind,indleft)
#     BI_left=BallImpurity(N,D,indleft,d)
#     BI_right=BallImpurity(N,D,indright,d)#BallImpurity(n,distance_matrix,split$rightInd,d)
#     bivec[i]=BI_full-m1/n*BI_left-m2/n*BI_right
#   }
#   return(sum(bi>bivec))
# }
...
#splitResult <- bestSplit(dataset, leafFunc=regLeaf, method="BallImpurity", ops=c(0,minLeafSize=5), lastSplitFeat, lossChoice="loss2")
#biTree <- buildTree(dataset, leafFunc=regLeaf, method='BallImpurity', ops = c(0, 4), maxDepth=4, leafDepth=1, lastSplitFeat=NULL, lossChoice='loss2') 
#pruneTree<-function(tree,method){
  #prune the tree using either pairwise distances, or impurity. 
#  }
